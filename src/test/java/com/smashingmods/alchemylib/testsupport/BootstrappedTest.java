package com.smashingmods.alchemylib.testsupport;
 ...
 * built-in registries, which the 1.20.6 {@code StreamCodec}s (for {@code ItemStack}/{@code Ingredient} content)
 * need to resolve registry holders while encoding and decoding.</p>
 */
public abstract class BootstrappedTest {
     ...
     * (and therefore {@code Ingredient}) by registry id refuse to run against a built-in registry whose
     * {@code doesSync()} is false, throwing {@code "Cannot use ID syncing for non-synced built-in registry"}.
     * NeoForge sets that flag during the mod-loading lifecycle, which {@link Bootstrap#bootStrap()} does not run,
     * so a bare bootstrap leaves the flag false and the id-based codecs unusable. Flipping it restores the sync
     * state a launched game has. The setter is package-private on NeoForge's {@code BaseMappedRegistry}, so it is
     * reached reflectively; that the registries are concrete {@code MappedRegistry} subclasses is fixed by the
     * platform.
     */
